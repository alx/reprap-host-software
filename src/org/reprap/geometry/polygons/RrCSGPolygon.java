...
public class RrCSGPolygon
{
    ...
    private String toString_r(int level)
    {
        if(q1 == null)
        {
            String result = "";
            for(int i = 0; i < level; i++)
                result = result + ".";
            result = result + Integer.toString(csg.complexity()) + "\n";
            return result;
        } else
        {
            level++;
            return(q1.toString_r(level) + 
                    q2.toString_r(level) +
                    q3.toString_r(level) +
                    q4.toString_r(level));
        }      
    ...
    public RrPolygon hatch_join(RrLine l0, double gap, int fg, int fs)
    {
        ...
        Rr2Point here = p_l.polygon(0).point(0);
        Rr2Point pa, pb;
